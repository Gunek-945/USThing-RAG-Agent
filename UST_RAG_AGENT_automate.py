...
def get_relevant_docs(user_input, namespace):
...
def remove_stopwords(text):
    ...
    text = " ".join(text)
    return text
#use_provided_namespace means using the namespace provided in the json file, different for each question
#provided_docs is the relevant docs for the first question, to avoid repeated calls to the retriever
#when providing docs, we don't care about the use_provided_namespace flag
def get_reponse(convo_entry:dict, provided_docs, use_provided_namespace):
    ...
    if not provided_docs:
        if use_provided_namespace:
            namespace= convo_entry['namespace']
        else:
            namespace=None
        docs = get_relevant_docs(user_input, namespace)
    else:
        docs= provided_docs
...
def test_with_json(json_file_path, same_question, use_provided_namespace=False):
    with open(json_file_path, 'r+') as file:
        data = json.load(file)
        conversations_list = data['conversations']
        if same_question:
            if use_provided_namespace:
                namespace= conversations_list[0]['namespace']
            else:
                namespace=None
            provided_docs = get_relevant_docs(conversations_list[0]['user_input'], namespace) #only retrieve once to avoid repeated calls 
        else:
            provided_docs= None
        for convo in conversations_list:
            try: 
                generation, response_time,docs= get_reponse(convo, provided_docs, use_provided_namespace)
...
if __name__=="__main__":
    ...
    test_with_json(r"Testing\test_regeneration.json", False, True)
